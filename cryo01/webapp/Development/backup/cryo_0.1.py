...
@app.route('/testing_prediction',methods=['GET','POST'])
def testing_prediction():
    ...
    if form.validate_on_submit():
        ...
        test_radio = form.test_radio.data

        f = form.test_image.data
        filename = secure_filename(f.filename)
        f.save(os.path.join(app.instance_path, 'photos', filename))

        filepath = str(app.instance_path)+"\\photos\\"+filename
    ...
    return render_template('testing_prediction.html',form=form,test_string=test_string,filename=filename,filepath=filepath)
