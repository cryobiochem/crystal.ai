...
#app.config['UPLOADED_IMAGES_DEST'] = 'static/images/test'
...
def cryo01_prediction():
    ...
    for model_file in model_path:
        if model_file[-2:] == 'h5':
            model = load_model('static/project_data/CRYO01/model/'+selected_model_dir+'/'+model_file)

        elif model_file == 'Label_dic.pkl':
            labels = joblib.load('static/project_data/CRYO01/model/'+selected_model_dir+'/'+model_file)
